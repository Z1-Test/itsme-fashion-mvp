...
import { createTemplateFragment, mergeTemplates, extractFlagKeys } from "../lib/template.js";
import { parseFrontmatter, parseFlagBlock } from "../lib/parser.js";
import { readFile, listMarkdownFiles, fileExists, getAccessToken, fetchTemplate, publishTemplate, resolveNamespace, log, } from "../lib/effects.js";
...
const parseCliArgs = (argv) => {
    ...
    const env = (getArg("env") ?? "dev-ecom-test");
 ...
 */
const getDefaultValueForEnv = (flag, env) => {
    if (env.includes("dev"))
        return flag.defaultDev;
    if (env.includes("stg"))
        return flag.defaultStg;
    return flag.defaultProd;
};
/**
 * Provision a single flag to all environments.
 */
const provisionFlag = (featureNumber, flagNumber, flag, boundedContext, description, accessToken, targetEnv, projectId, dryRun) => {
    ...
    const fragment = createTemplateFragment(boundedContext, flagKey, defaultValue, description, flag.type);
    // Merge and publish
...
const main = async () => {
    ...
    for (const filePath of files) {
        ...
        for (const flag of blockResult.flags) {
            ...
            const result = provisionFlag(featureNumber, flagNumber, flag, boundedContext, description, accessToken, args.env, projectId, args.dryRun);
            if (result.success) {
...
//# sourceMappingURL=sync-remote-config.js.map
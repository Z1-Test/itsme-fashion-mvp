 ...
 */
export const execGh = (args, dryRun = false) => {
    // Read operations should always execute even in dry run
    const isRead = args.includes("repo view") ||
...
// Temp File Helpers
// ============================================================================
const createTempFile = (prefix, content) => {
 ...
 */
export const getRepoInfo = (repoArg) => {
    if (repoArg) {
 ...
 */
export const getIssueTypes = (owner) => {
    try {
 ...
 */
export const setIssueType = (owner, repo, number, typeName, dryRun = false) => {
    if (!typeName)
        return false;
    try {
        const result = execGh(`api -X PATCH /repos/${owner}/${repo}/issues/${number} -f type="${typeName}"`, dryRun);
 ...
 */
export const createIssue = (request, dryRun = false) => {
    const { owner, repo, title, body } = request;
    const tempFile = createTempFile("issue_body", body);
    try {
        const result = execGh(`api -X POST /repos/${owner}/${repo}/issues -f title="${title}" --field body=@${tempFile}`, dryRun);
        if (dryRun) {
            return {
                number: 0,
                id: "dry-run",
 ...
 * Update issue body
 */
export const updateIssueBody = (number, body, dryRun = false) => {
    const tempFile = createTempFile("issue_body", body);
    try {
 ...
 * Update issue title
 */
export const updateIssueTitle = (number, title, dryRun = false) => {
 ...
 */
export const getDatabaseId = (nodeId) => {
    // If already a database ID (numeric string), return as-is
    if (/^\d+$/.test(String(nodeId))) {
        return String(nodeId);
    }
    const result = execGh(`api graphql -f query='query { node(id: "${nodeId}") { ... on Issue { databaseId } } }'`, false);
    if (!result)
        return null;
    try {
        const parsed = JSON.parse(result);
 ...
 */
export const linkSubIssue = (owner, repo, parentNumber, childNodeId, dryRun = false) => {
    const dbId = getDatabaseId(childNodeId);
    if (!dbId)
        return false;
    const result = execGh(`api repos/${owner}/${repo}/issues/${parentNumber}/sub_issues -X POST -F sub_issue_id=${dbId}`, dryRun);
    return dryRun || result !== null;
};
// ============================================================================
 ...
 */
export const createPullRequest = (title, body, base, head, dryRun = false) => {
    const tempFile = createTempFile("pr_body", body);
    try {
        const result = execGh(`pr create --title "${title}" --body-file "${tempFile}" --base ${base} --head ${head}`, dryRun);
        return dryRun || result !== null;
    }
    finally {
        deleteTempFile(tempFile);

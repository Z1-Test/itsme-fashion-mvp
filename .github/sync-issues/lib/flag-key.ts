...
export const deriveFlagKey = (
    ...
    `feature_fe_${featureNumber}_fl_${flagNumber}_${sanitizeContext(context)}_enabled`;
...
export const parseFlagKey = (
    key: string
): { featureNumber: number; flagNumber: number; context: string } | null => {
...
export const isValidFlagKey = (key: string): boolean =>
    /^feature_fe_\d+_fl_\d+_\w+_enabled$/.test(key);

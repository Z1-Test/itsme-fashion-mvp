...
const processEpic = (
    ...
    let renamed: RenameOperation | null = null;
...
const processFeature = (
    ...
    epicMap: Map<string, DocProcessResult>,
    dryRun: boolean
): DocProcessResult => {
    const file = basename(filePath);
    let content = readFileSync(filePath, "utf8");
    ...
    let issueUrl = fm.issue_url ?? null;
    let issueNumber = fm.issue_number ?? null;
    let issueId = (fm as Record<string, unknown>).issue_id as string ?? null;
    ...
    if (!issueUrl) {
        ...
        if (created) {
            ...
            if (flagType && issueNumber !== null && issueId !== null) {
                ...
                const context = (fm as Record<string, unknown>).flag_context as string ||
                    (flagBlock?.flags[0]?.context) ||
                    "default";

                flagIssueNumber = createFlagForFeature(
                    repoInfo,
                    issueNumber,
                    issueId,
                    context,
...
const linkToParentEpic = (
    ...
    // Find matching epic
    let parentEpicNumber: number | null = null;
...
const executeRenames = (
    ...
    fileToIssue: Map<string, DocProcessResult>,
    docsRoot: string
): void => {
...
const createCleanupPR = (dryRun: boolean): void => {
    ...
    const success = git.createAndPushBranch(
        branchName,
...
const main = async (): Promise<void> => {
    ...
    const epicMap = new Map<string, DocProcessResult>();
    ...
    const featureMap = new Map<string, DocProcessResult>();
    ...
    if (pendingRenames.length > 0 && !options.dryRun) {
        const combinedMap = new Map<string, DocProcessResult>([
